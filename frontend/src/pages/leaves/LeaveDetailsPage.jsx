...
import {
  Alert, Box, Button, Snackbar, Typography,
} from '@mui/material';
...
import {
  useLeave, useUpdateLeave, useDeleteLeave,
...
export default function LeaveDetailsPage() {
  ...
  return (
    <>
      <Helmet><title>Leave Details — Employee Portal</title></Helmet>

      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Button startIcon={<ArrowBackIcon />} onClick={() => navigate(ROUTES.LEAVES)} variant="text" aria-label="Back">
            Back
          </Button>
        ...
        {!isLoading && !isError && leave && (
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            {canEdit && isPending && (
              <Button variant="outlined" startIcon={<EditIcon />} onClick={() => setEditOpen(true)} aria-label="Edit">Edit</Button>
            )}
            {canApprove && isPending && (
              <Button variant="contained" color="success" startIcon={<CheckCircleIcon />}
                onClick={() => setApproveOpen(true)} aria-label="Approve">
                Approve
              </Button>
            )}
            {canApprove && isPending && (
              <Button variant="outlined" color="error" startIcon={<CancelIcon />}
                onClick={() => setRejectOpen(true)} aria-label="Reject">
                Reject
              </Button>
            )}
            {isPending && (
              <Button variant="outlined" color="error" startIcon={<DeleteIcon />}
                onClick={handleCancel} aria-label="Cancel request">
                Cancel Request
              </Button>
            )}
          </Box>
      ...
      {isError && (
        <Alert severity={error?.status === 404 ? 'warning' : 'error'} sx={{ mb: 2 }}>
          {error?.status === 404 ? 'Leave request not found.' : (error?.message ?? 'Failed to load details.')}
        </Alert>
      )}
